...
class Trepan:
	...
	def build_tree(MIN_EXAMPLES_PER_NODE,MAX_NODES,trainX,oracle):
		...
		#Uses the model to predict each row of trainX and store the label (1/0) in labels.
		...
		#Root Node takes two arguments: the dict (trainX and labels), and the number of rows
		root = Node(all_examples_dict,total_num_examples)
		#Priority is a combination of reach (fraction of examples/rows in this node), and number
		#of misclassifications (how many non-dominant classes are there)
		#For root, the reach is 1 (all examples are present). Number of misclassifications = no.
		#of non-dominant labels.
		#Constraints-> a class with number of dimensions (number of features) as variable
		...
		while not sortedQueue.empty():
			...
			num_examples=examples[0].shape[0]	#number of rows
			assert(node.leaf)
			assert(num_examples>0)

			print("############PROCESSING "+str(num_examples)+" #############")
			#If number of examples arrived at this node is lesser than the minimum allowed
			#(because if there are too few examples, it's an overfit)
			if num_examples<MIN_EXAMPLES_PER_NODE:
				print("NEED EXTRA")
				(trainX,labels)= examples
				#Find how many more examples are needed
				num_required = MIN_EXAMPLES_PER_NODE - num_examples
				#Generate synthetic examples using the Oracle using the constraint in this node
				(trainX_oracle,labels_oracle) = oracle.generate_constrained_examples_with_labels(constraints,num_required)
				#Augment these synthetic examples in the existing examples (call it examples_aug)
			...
			#Split using C4.5. Based on entropy.
...
class Oracle:
	...
	def __init__(self,network,num_classes,trainX):
		...
		#For each feature, get the feature distribution
	...
	def generate_feature_distributions(self,trainX):
		...
		for i in range(0,self.dimension):
			#Get ith column (feature) of trainX and reshape it
			feature_values = trainX[:,i].reshape(trainX.shape[0])
			feature_values+=0.0001
			kernel = stats.gaussian_kde(feature_values,bw_method='silverman')
			print("****")
			print(kernel)
			#For each feature, get the distribution and append it to this list
...
class Node:
	...
	def classify(self,example,layer1_width,layer2_width):
		'''
		...
		'''
		#frules=open('rules_german_pruned.txt','a')
		frules=open('../../temp/rules_gsp_balanced_2HL_200MAX.important.without_BR_ID.txt','a')

		if self.leaf :
			#frules.write("rule:"+str(self.splitrule.splits)+'\n')
			#if self.leaf.splitrule!=None:
			#print("rule:"+str(self.leaf.splitrule.splits))
			#frules.write("Arrived at leaf"+'\n')
			if layer2_width!=None:
				frules.write("Finally classified to:"+str(self.dominant)+' with layer1:'+str(layer1_width)+' and layer2:'+str(layer2_width)+'\n')
			else:
				frules.write("Finally classified to:"+str(self.dominant)+' with layer1:'+str(layer1_width)+'\n')
			print("------------------------------------------")
			return self.dominant
		if self.splitrule.satisfied(example):
			frules.write("rule:"+str(self.splitrule.splits)+'\n')
			#frules.write("Going left"+'\n')
			#if self.left_child.splitrule!=None:
			#	frules.write("rule:"+str(self.left_child.splitrule.splits)+'\n')
			return self.left_child.classify(example,layer1_width,layer2_width)
		else:
			frules.write("rule:"+str(self.splitrule.invert().splits)+'\n')
			#frules.write("Going right"+'\n')
			#if self.right_child.splitrule!=None:
			#	frules.write("rule:"+str(self.right_child.splitrule.splits)+'\n')
			return self.right_child.classify(example,layer1_width,layer2_width)
		frules.close()
